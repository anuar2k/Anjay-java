...
struct NativeAnjayObject {
    ...
    public:
        explicit Accessor(const jni::Object<NativeAnjayObject> &instance)
        ...
        std::string get_version() {
            return GlobalContext::call_with_env([&](auto &&env) {
                return jni::Make<std::string>(*env, get_method<jni::String()>(
                                                            "version")());
            });
        }
        ...
        int for_each_instance(Func &&func) {
            auto array_by_ref = GlobalContext::call_with_env([&](auto &&env) {
                return IntegerArrayByReference::New(*env);
            });
            int result =
            ...
            IntegerArrayByReference::for_each(array_by_ref, [&](int iid) {
        ...
        int for_each_resource_instance(anjay_iid_t iid,
            ...
            IntegerArrayByReference::for_each(array_by_ref, [&](int riid) {
                if (riid < std::numeric_limits<anjay_riid_t>::min()
                        || riid > std::numeric_limits<anjay_riid_t>::max()) {
                    avs_throw(IllegalArgumentException("riid out of range: "
                                                       + std::to_string(riid)));
                }
        ...
        int for_each_resource(anjay_iid_t iid, Func &&func) {
            auto array_by_ref = GlobalContext::call_with_env([&](auto &&env) {
                return ResourceDefArrayByReference::New(*env);
            });
            ...
            ResourceDefArrayByReference::for_each(array_by_ref, func);
            return 0;
        }
        ...
        int resource_read(anjay_iid_t iid,
            ...
            return get_method<jni::jint(
                    ...
                    "resourceRead")(
                    iid, rid, riid,
                    utils::NativeOutputContextPointer::into_object(ctx));
        }
        ...
        int resource_write(anjay_iid_t iid,
            ...
            return get_method<jni::jint(
                    ...
                    "resourceWrite")(
                    iid, rid, riid,
                    utils::NativeInputContextPointer::into_object(ctx));
        }

        int resource_execute(anjay_iid_t iid,
                             anjay_rid_t rid,
                             anjay_execute_ctx_t *ctx) {
            return GlobalContext::call_with_env([&](auto &&env) -> int {
                auto clazz = jni::Class<HashMap>::Find(*env);
                auto ctor = clazz.template GetConstructor<>(*env);
                auto args_map = clazz.New(*env, ctor);

                auto accessor = AccessorBase<HashMap>{ args_map };
                auto args_map_inserter = accessor.get_method<jni::Object<>(
                        jni::Object<>, jni::Object<>)>("put");
                ...
                while (!(retval = anjay_execute_get_next_arg(ctx, &current_arg,
                    ...
                    if (has_value) {
                        ...
                        opt = utils::Optional::empty().into_java();
                    }
                    args_map_inserter(jni::Box(*env, current_arg), opt);
                }
                ...
                return get_method<jni::jint(jni::jint, jni::jint,
                        ...
                        "resourceExecute")(
                        iid, rid,
                        jni::Cast(*env, jni::Class<Map>::Find(*env), args_map));
            });
        }
        ...
        int object_read_default_attrs(anjay_ssid_t ssid,
            ...
            auto attrs_by_reference =
                    GlobalContext::call_with_env([&](auto &&env) {
                        return utils::ObjectInstanceAttrsByReference::New(*env);
                    });
            int result = get_method<jni::jint(
                    jni::jint,
            ...
            if (!result) {
                auto attrs = utils::ObjectInstanceAttrsByReference::get_value(
                        attrs_by_reference);
                *out = utils::ObjectInstanceAttrs::into_native(attrs);
            }
            return result;
        }
        ...
        int object_write_default_attrs(anjay_ssid_t ssid,
            ...
            return get_method<jni::jint(
                    ...
                    ssid, utils::ObjectInstanceAttrs::into_java(attrs));
        }
        ...
        instance_write_default_attrs(anjay_iid_t iid,
            ...
            return get_method<jni::jint(
                    ...
                    iid, ssid, utils::ObjectInstanceAttrs::into_java(attrs));
        }
        ...
        int resource_read_attrs(anjay_iid_t iid,
            ...
            auto attrs_by_reference =
                    GlobalContext::call_with_env([&](auto &&env) {
                        return utils::ResourceAttrsByReference::New(*env);
                    });
            int result = get_method<jni::jint(
                    jni::jint, jni::jint, jni::jint,
            ...
            if (!result) {
                auto attrs = utils::ResourceAttrsByReference::get_value(
                        attrs_by_reference);
                *out = utils::ResourceAttrs::into_native(attrs);
            }
            return result;
        }
        ...
        int resource_write_attrs(anjay_iid_t iid,
            ...
            return get_method<jni::jint(jni::jint, jni::jint, jni::jint,
                    ...
                    "resourceWriteAttrs")(
                    iid, rid, ssid, utils::ResourceAttrs::into_java(attrs));
        }
        ...
        resource_instance_write_attrs(anjay_iid_t iid,
            ...
            return get_method<jni::jint(jni::jint, jni::jint, jni::jint,
                    ...
                    iid, rid, riid, ssid,
                    utils::ResourceAttrs::into_java(attrs));
        }
    };
};
